...
def generate_response(prompt):
    # Limit the context length to 14000 tokens
    max_tokens = 14000
    current_tokens = sum(len(message['content']) for message in prompt)

    # Truncate messages if they exceed the token limit
    while current_tokens > max_tokens:
        if len(prompt) > 1:
            prompt.pop(1)  # Remove the oldest user message
            current_tokens = sum(len(message['content']) for message in prompt)
        else:
            # If there's only one message left, truncate it directly
            prompt[0]['content'] = prompt[0]['content'][:max_tokens]
...
class CharacterApp:
    def __init__(self, root):
        ...
        hex_color = "#94fc9f"  # Light blue color as an example
